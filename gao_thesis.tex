...
\begin{algorithm}
 \While{Policy not converged ($\pi_t \neq \pi_{t+1}$)}{
		\textbf{Explore:} 
		Sample $\bsym{\theta}$ to generate one policy $\pi_t$\;
		Use policy $\pi_t$ to generate trajectory $\bsym{\gamma}$ and receive cumulative reward $r$;
		
		\textbf{Evaluate:}
		Evaluate the policy based trajectory $\bsym{\gamma}$ and reward $r$.;
		
		\textbf{Update:}
		Update the $\bsym{\theta}$ using optimization method.;
 }
 \caption{Gneral Framework of Policy Gradient Algorithm}\label{policy_gradient}
\end{algorithm}
\subsection{Learning Method of Policy Gradient Method}
The philosophy behind Policy gradient methods are different from traditional reinforcement learning methods. As a consequence, the learning method is also different from traditional reinforcement learning. Although the policy gradient method can also be learned in a step-by-step fashion, they prefer episodic updating rules.\cite{Stulp2012}\cite{Sun2009}\cite{Daniel2012}. This leads to the use of cumulative reward of system:

\begin{equation}
R=R(\theta)=\mathbb{E}_{p_{\theta}(\gamma)}\left[\sum_{t=0}r^t|\theta\right]
\end{equation}

However this method suffers from the variance of reward $R$. Due to different configurations of initial state, noise of system and stochasticity of system, few evaluation of policy parameters does not always show true gradient direction of the optimization.  Hence, in practice, the policy $\pi$ is always modelled as deterministic to avoid perturbation in optimization phase.

Optimization of policy gradient methods are also different. As the process of generating reward is not known, the optimization can only be done by 

\subsection{Policy Gradient Based on Parameter Exploration}
